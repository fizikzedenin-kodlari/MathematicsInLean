...
theorem div'_add_mod' (a b : ℤ) : b * div' a b + mod' a b = a := by
  rw [div', mod']
  linarith [Int.ediv_mul_add_emod (a + b / 2) b]
------------------------------------------------
theorem abs_mod'_le (a b : ℤ) (h : 0 < b) : |mod' a b| ≤ b / 2 := by
  ...
  have := Int.ediv_mul_add_emod b 2
...
------------------------------------------------
theorem sq_add_sq_eq_zero {α : Type*} [Ring α] (x y : α) :
...
theorem conj_im (x : GaussInt) : (conj x).im = -x.im :=
  rfl
------------------------------------------------
theorem norm_conj (x : GaussInt) : norm (conj x) = norm x := by sorry
------------------------------------------------
...
theorem div_def (x y : GaussInt) :
    x / y = ⟨div' (x * conj y).re (norm y), div' (x * conj y).im (norm y)⟩ :=
  rfl
------------------------------------------------
theorem mod_def (x y : GaussInt) : x % y = x - y * (x / y) :=
  rfl
-------**This code has a lot of bugs. I'll update it once I fix them.**---------------
...
--      _ ≤ (y.norm / 2) ^ 2 + (y.norm / 2) ^ 2 := by gcongr <;> apply Int.abs_mod'_le _ norm_y_pos
